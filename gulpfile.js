...
gulp.task("babel", gulp.series(function () {
    return gulp.src(["src/**/*.js", "!src/web-app/**"])
        .pipe(babel())
        .pipe(gulp.dest("dist"));
}));
...
gulp.task("clone-react-app", gulp.series(function () {
    return gulp.src("src/web-app/build/**/*")
        .pipe(gulp.dest("dist/web-app"));
}));

gulp.task('sass', gulp.series(function () {
    return gulp.src('./src/web-app/scss/style.scss')
        .pipe(sass().on('error', sass.logError))
        .pipe(concat('style.css'))
        .pipe(gulp.dest('./src/web-app/public/css'))
...
gulp.task('sass:watch', gulp.series(function () {
    gulp.watch('./src/web-app/scss/**/*.scss', gulp.series('sass'));
}));
